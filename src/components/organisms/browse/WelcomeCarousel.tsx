      ...
      {(currentSlide.title || currentSlide.description || currentSlide.actionType !== 'none') && (
        <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10 md:p-12 flex flex-col justify-end max-w-2xl text-left z-10">
          {currentSlide.mediaType && (
            <span
              className="mb-2 inline-block rounded-full bg-blue-600/90 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-blue-100 border border-blue-500/40 self-start shadow-md"
      ...
      {slides.length > 1 && (
        <div className="absolute right-6 bottom-6 z-10 flex gap-2 sm:right-10 sm:bottom-10">
          <button
            ...
            tabIndex={-1}
            className="flex h-10 w-10 items-center justify-center rounded-xl border border-gray-800 bg-gray-950/70 text-gray-400 transition-all duration-200 hover:border-gray-600 hover:text-white"
          >
            <ChevronLeft size={18} />
          </button>
          <button
