...
- `src/components/organisms/discover/CategorySelector.tsx` — the sidebar/strip, marked `data-focus-group="categories"` with `data-selected="true"` on the active category button, which the up-arrow-leaving-grid logic specifically looks for. (`src/components/organisms/admin/DraggableCategoryList.tsx` only marks `data-selected` — it's not part of this focus group.)
