...
### 5. `useMediaLibrary.ts`'s initial-load effect — never fired for accounts that had never saved a preference

Reported symptom: "default page not loading correctly when logging in" — landed on a blank/stuck-empty grid, disproportionately for regular user accounts rather than admin ones. The initial-load effect (comment block "1. Initial Load of main content") gated on `!providerKey || !user?.preferences` — the assumption being that a falsy `preferences` meant "profile fetch hasn't landed yet," a transient state. It isn't: the backend only creates a `preferences` row the first time something actually saves one (see portalcast-server's `skill-user-system.md`), so an account that has never changed a setting has `preferences` null/undefined **permanently**, not briefly. Admin accounts tend to have saved *something* already (more time in the admin panel), which is why the symptom skewed toward regular users rather than being universal. The effect simply never ran for those accounts — stuck until some unrelated tab switch happened to create the preferences record as a side effect.

**Fix**: the gate is now just `!providerKey || !user`; missing `preferences` falls back to `{}` and each field reads its own default off that (`prefs.favorites || []`, `prefs.preferredContentType || 'movie'`, etc.) instead of blocking on the object's existence. Separately, `fetchData` now returns a `boolean` (success/failure) instead of `void`, and `restoredForRef` — the ref that prevents this effect from re-running for the same user+provider — is only latched **after** a successful fetch, not before it starts. Previously a single failed/out-of-order first attempt (e.g. racing the auth token still settling right after login) marked restoration "done" regardless of outcome, leaving the page stuck empty for the rest of the session since nothing else re-triggered the effect. On failure it now retries once automatically after a 1.5s delay before giving up.

**Second, real cause of the same symptom (found later)**: the effect is also gated on `providerKey`, and that used to come from `GET /api/config`. The server's global `onPreHandler` treats every `/api/config*` path as admin-only (and the handler returns the full config, credentials included), so for regular users the call 403'd, `providerKey` stayed `''`, and the initial load never fired — the page only filled once a tab switch called `fetchData` directly. Admins never saw it. `providerKey` now comes from `getProviderKey()` (`src/api/endpoints/provider.ts` → `GET /api/v2/provider-key`, any logged-in user, returns only the key in the same `{type}_{host}` shape so saved per-provider preference keys keep matching). The effect's deps also use `user?.id` rather than the whole `user` object — every `refreshProfile()` replaces the object and was cancelling the in-flight first fetch. **Don't read `/config` from non-admin code paths.**
...
- `src/hooks/useMediaLibrary.ts` — `fetchData` (returns `boolean` success/failure, see bug #5 above), `fetchRequestRef`, `restoredForRef`. The silent artwork-refresh interval is `src/hooks/useLibraryBackgroundRefresh.ts`; the fill-viewport/scroll-listener "load more" pair is `src/hooks/useInfiniteScroll.ts`.
